...
func intersections(wire1 []coords, wire2 []coords) []intersect {
	crosses := []intersect{}

	for s1, c1 := range wire1 {
		for s2, c2 := range wire2 {
			if c1.x == c2.x && c1.y == c2.y {
				crosses = append(crosses, intersect{x: c1.x, y: c1.y, step: s1 + s2 + 2}) // add 2 because s1 and s2 are zero-based
			}
		}
	}
...
func trace(lattice []string) []coords {
	x := 0
	y := 0
	c := []coords{}
